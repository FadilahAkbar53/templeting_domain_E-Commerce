...
const AdminPage: React.FC = () => {
  ...
  const handleSave = async (productData: Product | Omit<Product, '_id'>) => {
    try {
      ...
      await fetchProducts(); // Refetch products after saving
    } catch (err) {
    ...
    <>
      <div className="bg-theme-bg-primary shadow-lg rounded-xl p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-6 gap-4">
          <h1 className="text-3xl font-bold text-theme-text-base">Product Management</h1>
          <button
            ...
            <table className="w-full text-sm text-left text-theme-text-muted">
              ...
              <tbody>
                {products.map((product) => (
                  <tr key={product._id} className="bg-theme-bg-primary border-b border-theme-border hover:bg-theme-bg-secondary">
                    <td className="p-4">
                      <img src={product.image} alt={product.name} className="w-16 h-16 object-cover rounded-md" />
                    </td>
                    <td className="px-6 py-4 font-semibold text-theme-text-base whitespace-nowrap">{product.name}</td>
                    <td className="px-6 py-4">{product.brand}</td>
                    <td className="px-6 py-4 font-semibold">{formatCurrency(product.price)}</td>
                    <td className="px-6 py-4 text-right space-x-2">
                      <button onClick={() => handleEdit(product)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</button>
                      <button onClick={() => handleDelete(product._id)} className="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
